...
import { Plus, TrendingUp, TrendingDown, ArrowUpDown, Search } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Registre entradas e saídas do estoque</p>
        </div>
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="bg-card border-border">
            <DialogHeader>
              <DialogTitle className="text-foreground">Registrar Movimentação</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <Button
                  type="submit"
                  className={cn(
                    formData.type === "entrada" ? "bg-chart-2 hover:bg-chart-2/90" : "bg-primary hover:bg-primary/90",
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        >
          <CardContent className="flex items-center gap-4 p-6">
            ...
            <div>
              <p className="text-lg font-semibold text-foreground">Entrada Rápida</p>
              <p className="text-sm text-muted-foreground">Adicionar itens ao estoque</p>
            </div>
          </CardContent>
        </Card>
        ...
        >
          <CardContent className="flex items-center gap-4 p-6">
            ...
            <div>
              <p className="text-lg font-semibold text-foreground">Saída Rápida</p>
              <p className="text-sm text-muted-foreground">Retirar itens do estoque</p>
            </div>
          </CardContent>
        </Card>
      </div>
                  ...
                  filteredMovements.map((movement) => (
                    <TableRow key={movement.id} className="border-border hover:bg-secondary/50">
                      <TableCell>
                        <div
                          className={cn(
                            ...
                            movement.type === "entrada" ? "bg-chart-2/20 text-chart-2" : "bg-primary/20 text-primary",
                          )}
                        >
                      ...
                      <TableCell className="text-center">
                        <span
                          className={cn("font-bold", movement.type === "entrada" ? "text-chart-2" : "text-primary")}
                        >
                          {movement.type === "entrada" ? "+" : "-"}
                          {movement.quantity}
                        </span>
                      ...
                      <TableCell className="text-muted-foreground text-sm">{formatDate(movement.date)}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
