...
import { Factory, Plus, Pencil, Trash2, Search, Phone, Mail, User } from "lucide-react"
import { useStockStore, type Supplier } from "@/lib/store"
...
export function Suppliers() {
  const { suppliers, addSupplier, updateSupplier, deleteSupplier, products } = useStockStore()
  ...
  const SupplierForm = ({
    data,
    setData,
  }: { data: typeof emptySupplier | Supplier; setData: (data: typeof emptySupplier | Supplier) => void }) => (
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie os fornecedores de peças</p>
        </div>
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          ...
          <DialogContent className="bg-card border-border max-w-2xl">
            <DialogHeader>
              <DialogTitle className="text-foreground">Adicionar Fornecedor</DialogTitle>
              <DialogDescription>Cadastre um novo fornecedor no sistema</DialogDescription>
            </DialogHeader>
            <SupplierForm
              data={newSupplier}
              setData={setNewSupplier as (data: typeof emptySupplier | Supplier) => void}
            />
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsAddOpen(false)}>
                Cancelar
              </Button>
              <Button onClick={handleAddSupplier} className="bg-primary hover:bg-primary/90">
                Adicionar
              </Button>
            </DialogFooter>
          </DialogContent>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  ...
                  <TableHead className="text-muted-foreground hidden lg:table-cell">Responsável</TableHead>
                  <TableHead className="text-muted-foreground text-center hidden sm:table-cell">Produtos</TableHead>
                  ...
                  <TableRow key={supplier.id} className="border-border">
                    ...
                    <TableCell className="hidden lg:table-cell">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-foreground">{supplier.contact || "-"}</span>
                      </div>
                    </TableCell>
                          ...
                          <DialogContent className="bg-card border-border max-w-2xl">
                            ...
                            {editingSupplier && (
                              <SupplierForm
                                ...
                                setData={setEditingSupplier as (data: typeof emptySupplier | Supplier) => void}
                              />
                            )}
                            <DialogFooter>
                              ...
                              <Button onClick={handleUpdateSupplier} className="bg-primary hover:bg-primary/90">
                                Salvar
                              </Button>
                            </DialogFooter>
                          </DialogContent>
