...
import { attachOrdensVendaPecas } from '../_lib/order-relations';
...
export async function GET(request: Request) {
  ...
  if (ordemIds.length > 0) {
    todasPecas = await db
      .select({
        ordem_venda_id: schema.ordemVendaPecas.ordem_venda_id,
      ...
      .where(inArray(schema.ordemVendaPecas.ordem_venda_id, ordemIds));
  }

  const ordensComPecas = attachOrdensVendaPecas(ordens, todasPecas);
...
export async function POST(request: Request) {
  ...
  if (!data.cliente_id) {
    return new Response(JSON.stringify({ error: 'Cliente é obrigatório' }), {
      status: 400
    });
  }
  ...
  try {
    ...
    await logAction(
      ...
      'ordem_venda',
      String(result.id),
      `Ordem de venda #${result.id} criada`
    );
    return new Response(JSON.stringify(result));
  } catch (error) {
    const message =
      error instanceof Error ? error.message : 'Erro ao criar ordem de venda';
