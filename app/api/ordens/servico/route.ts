...
import { attachOrdensServicoRelations } from '../_lib/order-relations';
...
export async function GET(request: Request) {
  ...
  if (ordemIds.length > 0) {
    [todasPecas, todasMaoObra] = await Promise.all([
      db
        .select({
          ordem_servico_id: schema.ordemServicoPecas.ordem_servico_id,
        ...
        .where(inArray(schema.ordemServicoPecas.ordem_servico_id, ordemIds)),

      db
        .select({
          ordem_servico_id: schema.ordemServicoMaoObra.ordem_servico_id,
        ...
        .where(inArray(schema.ordemServicoMaoObra.ordem_servico_id, ordemIds))
    ]);
  }

  const ordensComPecas = attachOrdensServicoRelations(
    ordens,
    todasPecas,
    todasMaoObra
  );
...
export async function POST(request: Request) {
  ...
  if (
    !data.funcionario_id ||
    !data.funcionario_responsavel_id ||
    !data.cliente_id ||
    !data.veiculo_id ||
    !data.data_chegada
  ) {
    return new Response(
      JSON.stringify({
        error:
          'Campos obrigatórios faltando (cliente, veículo, funcionários ou data de chegada)'
      }),
      { status: 400 }
    );
  }
  ...
  try {
    ...
    await logAction(
      ...
      'ordem_servico',
      String(result.id),
      `Ordem de serviço #${result.id} criada`
    );
    ...
    const message =
      pgError.detail || pgError.message || 'Erro ao criar ordem de serviço';
    return new Response(
      JSON.stringify({ error: message, code: pgError.code }),
      { status: 400 }
    );
  }
}
